...
class MetropolisHastingsSampler(Sampler):
	...
	# def sequential_sample(self, curr_data, curr_sample, skip_iter = 1):
	# 	"""Generate another sample."""
	# 	for _ in range(skip_iter+1):
	# 		proposal, fb = self.proposer(curr_sample)

	# 		assert proposal is not curr_sample, "*** Proposal cannot be the same as the current sample!"
	# 		assert proposal.value is not curr_sample.value, "*** Proposal cannot be the same as the current sample!"

	# 		proposal.compute_posterior(curr_data, shortcut=-Infinity)
	...
	# 		prop = (proposal.prior/self.prior_temperature +
	# 				proposal.likelihood/self.likelihood_temperature)
	# 		cur = (curr_sample.prior/self.prior_temperature +
	# 				curr_sample.likelihood/self.likelihood_temperature)

	# 		if self.trace:
	# 			print("# Current: ", round(cur,3), curr_sample)
	# 			print("# Proposal:", round(prop,3), proposal)
	# 			print("")
	...
	# 			curr_sample = proposal

	# 	return curr_sample

	def sequential_sample(self, curr_data, skip_iter = 1):
		"""Generate another sample."""
		for _ in range(skip_iter+1):
			...
			self.compute_posterior(self.proposal, curr_data)

			print("sequential sampling")
			print(self.proposal.likelihood)
			print(self.current_sample.likelihood)
...
class Acontextual_MHS(Sampler):
	def __init__(self, current_sample, steps = Infinity, proposer = None, skip = 0, 
	      	prior_temperature = 1.0, likelihood_temperature = 1.0, acceptance_temperature = 1.0, trace = False, shortcut_likelihood = True):
		self_update(self, locals())
		self.was_accepted = None
		...
		self.samples_yielded = 0
		self.set_state(current_sample, compute_posterior = False)
		self.reset_counters()
	...
	def sequential_sample(self, curr_data, skip_iter = 1):
		...
		for _ in range(skip_iter+1):
			...
			# Call myself so memoized subclasses can override
			self.compute_posterior(self.current_sample, curr_data)
			self.compute_posterior(self.proposal, curr_data)
